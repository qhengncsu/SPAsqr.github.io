...
A single file may carry both phenotype and covariate columns (as here), or the two may live in separate files supplied through `--pheno` and `--covar`. GRAB also accepts a single `IID` key column in place of the `FID IID` pair; LDAK-KVIK and REGENIE require the `FID IID` pair. All four input files (`1kg.{bed,bim,fam}` and `1kg.pheno`) plus the `grab2` binary are available in the [`examples/`](https://github.com/qhengncsu/GRAB-feat-cpp/tree/main/examples) folder of our [GRAB GitHub repository](https://github.com/qhengncsu/GRAB-feat-cpp) (also linked at the top-right of this page); users may download them to replicate this tutorial verbatim.
